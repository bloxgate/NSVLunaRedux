/obj/item/device/spacepod_equipment/weaponry/proc/fire_weapons()
	...
	if(chassis.next_firetime > world.time)
		usr << "<span class='warning'>Your weapons are recharging.</span>"
		return
	var/turf/firstloc
	var/turf/secondloc
	if(!chassis.equipment_system || !chassis.equipment_system.weapon_system)
		usr << "<span class='warning'>Missing equipment or weapons.</span>"
		chassis.verbs -= text2path("[type]/proc/fire_weapons")
		return
	chassis.battery.use(shot_cost)
	var/olddir
	for(var/i = 0; i < shots_per; i++)
		...
		sleep(1)
	chassis.next_firetime = world.time + fire_delay
