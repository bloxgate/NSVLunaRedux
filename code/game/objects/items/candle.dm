...
/obj/item/weapon/flame/candle/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/weldingtool))
		...
		if(WT.isOn()) //Badasses dont get blinded by lighting their candle with a welding tool
			light(user,"\red [user] casually lights the [name] with [W].")
...
/obj/item/weapon/flame/candle/proc/light(mob/user as mob, var/flavor_text = "\red [usr] lights the [name].")
	if(!src.lit)
		src.lit = 1
		//src.damtype = "fire"
...
/obj/item/weapon/flame/candle/on_enter_storage()
	if(lit)
		lit = 0
		update_icon()
		usr.AddLuminosity(-2,-1,0)
	..()
	return